    ...
    <div class="rank-grid">
      <div>
        ...
        <div class="rank-score">
          <strong>${numberFormat(data.mainScore)}</strong><span>主积分</span>${
            (data.bonusScore ?? 0) > 0
              ? ` <span class="bonus-pill">+${numberFormat(data.bonusScore)} 彩蛋</span>`
              : ""
          }
        </div>
      </div>
...
function renderExtraRanks() {
  ...
  const items = [
    ...
    ["全班作业分榜", data.extraRanks.classHomework]
  ];
...
function renderStats() {
  ...
  const highestPretestScore = data.stats.highestPretestScore ?? data.stats.pretestScore ?? 0;
  const items = [
    {
      label: "完成率",
    ...
    {
      label: "学前测",
      value: data.stats.pretestScore == null ? "--" : `${numberFormat(data.stats.pretestScore, 0)}分`,
      notes: ["课前基础", `最高分 ${numberFormat(highestPretestScore, 0)}分`]
    },
    {
      label: "出勤",
  ...
  const breakdown = document.getElementById("growth-breakdown");
  breakdown.innerHTML = (data.growthBreakdown ?? [])
    .map((item) => {
      const partsHint = Array.isArray(item.parts) && item.parts.length > 0
        ? `<div class="rule-subtext">${item.parts.map((p) => `${p.label} +${p.score}`).join(" · ")}</div>`
        : "";
      return `
        <div class="rule-row rule-row-split">
          <div>
            <span>${item.label}</span>
            <div class="rule-subtext">当前 ${numberFormat(item.rawValue)} / 基准 ${numberFormat(item.benchmark)}，还差 ${numberFormat(item.gap)} 分可满分</div>
            ${partsHint}
          </div>
          <strong>${numberFormat(item.actualScore)} / ${numberFormat(item.fullScore)} 分</strong>
        </div>
      `;
    })
    .join("");

  const mainScore = data.mainScore ?? 0;
  const bonusScore = data.bonusScore ?? 0;
  const finalScore = data.rankInfo?.growthScore ?? 0;
  const summaryEl = document.getElementById("growth-score-summary");
  if (summaryEl) {
    summaryEl.innerHTML =
      `主积分 <strong>${numberFormat(mainScore)}</strong> / 100` +
      (bonusScore > 0
        ? ` + 彩蛋 <strong>${numberFormat(bonusScore)}</strong>`
        : "") +
      ` = <strong>${numberFormat(finalScore)}</strong> / 100`;
  }

  const bonusSection = document.getElementById("bonus-section");
  if (bonusSection) {
    if (Array.isArray(data.bonusBreakdown) && data.bonusBreakdown.length > 0) {
      const hitCount = data.bonusBreakdown.filter((b) => b.hit).length;
      const bonusTotal = data.bonusBreakdown.reduce((s, b) => s + b.score, 0);
      const bonusMax = data.bonusMax ?? bonusTotal;
      bonusSection.classList.remove("hidden");
      const bonusList = document.getElementById("bonus-breakdown");
      bonusList.innerHTML = data.bonusBreakdown
        .map(
          (b) => `
            <div class="rule-row rule-row-split ${b.hit ? "bonus-hit" : "bonus-miss"}">
              <div>
                <span>${b.label}</span>
                <div class="rule-subtext">${b.hit ? "🎉 已达成" : "未达成，继续加油"}</div>
              </div>
              <strong>+${numberFormat(b.score)} / +${numberFormat(b.fullScore)} 分</strong>
            </div>
          `
        )
        .join("");
      ...
      if (summary) {
        summary.textContent = `共命中 ${hitCount} / ${data.bonusBreakdown.length} 项彩蛋，累计 +${numberFormat(bonusScore)} 分（满分 +${numberFormat(bonusMax)}）`;
      }
    } else {
...
function renderRankRows(containerId, rows, valueLabel = "成长积分", valueKey = "score") {
  ...
  container.innerHTML = rows
    .map((item) => {
      ...
      const tiedCount = Number(item.tiedCount ?? 0);
      const tiedHint =
        tiedCount > 1
          ? `<span class="tied-tag" title="本分数并列人数">共 ${tiedCount} 人同分</span>`
          : "";
      return `
        <div class="${rowClass}">
...
function renderHomeworkTop10() {
  renderRankRows("homework-top10", data.homeworkTop20 ?? [], "分", "value");
}
